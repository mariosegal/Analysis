...
where &condition;
 deposits = min(sum(dda,mms,sav,tda,ira),1);
	loans = min(sum(iln,ind,mtg, heq, sln,card),1);
	securities = min(sum(sec,ins),1);
run;


/*penetration and Balances*/
Proc tabulate data=temp out=prod1;
class virtual_seg RM;
...
table  virtual_seg*RM ALL, hh*sum='HH Count'*f=comma12. (sum='Product HHs'*f=comma12.0)*(dda mms sav tda ira sec trs ins mtg heq card ILN ind sln sdb) 
(pctsum<hh>='Product Penetration'*f=PCTPIC.)*(dda mms sav tda ira sec trs ins mtg heq card ILN ind sln sdb )  
...
(DDA_Amt MMS_amt sav_amt TDA_Amt IRA_amt sec_Amt trs_amt MTG_amt HEQ_Amt ccs_Amt iln_amt ind_amt sln_amt )*mean='Avg. Per Prod HH'*f=dollar12. / nocellmerge;
keylabel PCTSUM = 'PCT';
...
proc sort data=prod3;
by descending _type_   virtual_seg RM;
run;

proc print data=prod3 (drop=_type_ _page_ _table_) noobs;
run;
...
data temp_hh;
set temp (keep= hhid hh dda mms sav tda ira ins sec trs mtg heq card iln sln ind virtual_seg segment  rm);
run;
...
data temp_contrib ;
merge  data.Contrib_201111 (in=b) temp_hh (in=a);
by hhid;
if a and b;
run;

Proc tabulate data=temp_contrib out=contr1;
class virtual_seg RM;
...
table  virtual_seg*RM ALL, hh*sum='HHs'*f=comma12. (sum='Product Penetration'*f=comma12.0)*(dda mms sav tda ira sec trs ins mtg heq card ILN ind sln ) 
(sum='Total Contribution'*f=dollar12.)*(DDA_CON MMS_CON sav_CON TDA_CON IRA_CON sec_CON trs_CON MTG_CON HEQ_CON caRD_CON iln_CON ind_CON sln_CON ) 
(mean='Avg. Per Product HH'*f=dollar12.2)*(DDA_CON MMS_CON sav_CON TDA_CON IRA_CON sec_CON trs_CON MTG_CON HEQ_CON cARD_CON iln_CON ind_CON sln_CON ) / nocellmerge;
/*format segment segfmt.;*/
run;
...
data temp_band;
set temp;
keep hhid hh rm virtual_seg band ;
run;

Proc tabulate data=temp_band out=band1;
class virtual_seg rm band;
var hh;
table (band )* (N*f=comma12. ), virtual_seg*(rm ALL) /nocellmerge ;
...
length wealth $ 15;
	...
	assets = x;
drop x;
keep hhid hh cqi: wealth virtual_seg segment clv: assets rm;
run;

proc tabulate data=temp_1 ;
class virtual_seg rm;
...
table hh*f=comma12.0 assets*mean*f=dollar12.0 (clv_total clv_rem)*mean*f=dollar12.0 clv_rem_ten*mean*f=comma8.1 
      cqi*mean*f=comma5.1 (cqi_bp cqi_DD cqi_deb cqi_odl cqi_web )*pctsum<hh>*f=PCTPIC.,virtual_seg*(rm ALL) /nocellmerge ;
/*format segment segfmt.;*/
run;


data temp_demog;
merge temp_1 (in=a keep=hhid hh virtual_seg segment rm) data.demog_201111 (in=b);
by hhid;
if a and b;
run;
...
table hh*f=comma12.0 own_age*mean*f=comma5.1 (child under_10 child_11_15 child_16_17)*pctsum<hh>*f=pctpic.  
      ,virtual_seg*(rm ALL) /nocellmerge ;
format segment segfmt.;
run;


proc tabulate data=temp_demog;
class virtual_seg rm own_age ;
var hh ;
table  hh own_age 
      ,virtual_seg*(rm ALL) /nocellmerge ;
...
proc transpose data=tempq out=temp_chk_2 (drop=_NAME_);
by hhid;
id stype;
run;
...
data chk_merged;
merge temp (in=a keep=hhid hh segment virtual_seg rm) temp_chk_2;
by hhid;
if a;
run;


proc tabulate data=chk_merged (rename=(rm=frm));
class frm virtual_seg;
var hh R:;
table hh*sum*f=comma12.0 (R:)*N (R:)*pctN<hh> ,virtual_seg*(fRM ALL) /nocellmerge;
run;
...
vpos mpos atmo atmt br_tr vru web1 bp1 sms1 wap1 fico1 fworks1 chk1 rm; 
run;
...
table    (hh = 'HHs' web = 'Web Enr'  web1 = 'Web Act' bp = 'BPay Enr.' bp1 = 'BPay Act'  edeliv = 'e-deliv Enr'  estat = 'e-statem Enr.'  
		...
		(vpos_amt = 'Deb Sign' mpos_amt = 'Deb PIN' atmo_amt = 'MT ATM' atmt_amt = 'oth ATM' bp_amt = 'Bill Pay'  
         ...
         , virtual_seg='Transaction Segment'*rm;
format segment segfmt.; 
run;


Proc tabulate data=temp out=prod9;
class virtual_seg rm;
...
(DDA_Amt MMS_amt sav_amt TDA_Amt IRA_amt sec_Amt trs_amt MTG_amt HEQ_Amt ccs_Amt iln_amt ind_amt sln_amt )*sum='Total Balances'*f=dollar12. 
(DDA_Amt MMS_amt sav_amt TDA_Amt IRA_amt sec_Amt trs_amt MTG_amt HEQ_Amt ccs_Amt iln_amt ind_amt sln_amt )*mean='Avg. $ Per Total HH'*f=dollar12.,(virtual_seg ALL)*(rm ALL) ALL;
/*keylabel PCTSUM = 'PCT';*/
format segment segfmt.;
run;
