...
run;
proc tabulate data=data.main_201203 order=data out=penet missing;
where groupid ne '' and groupid ne '0';
where groupid ne '' and groupid ne '0';
var dda: mms: tda: sav: mtg: heq: iln: ccs: ira: trs: sec: card ccs_amt ins ind: sln: hh sdb: ;
CLASS groupid / PRELOADFMT;
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (dda='Checking' mms='Mon Mkt' sav='Savings' tda='Time Dep' ira='IRA' mtg='Mortgage' heq='Home Eq' card='Credit Card' iln='Dir Loan' ind='Ind Loan' sln='Student loan' sec='Securities' ins='Insurance'
...
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (dda_amt='Checking'*pctsum<dda>='Avg. Bal.' mms_amt='Mon Mkt'*pctsum<mms>='Avg. Bal.' sav_amt='Savings'*pctsum<sav>='Avg. Bal.' tda_amt='Time Dep'*pctsum<tda>='Avg. Bal.'
...
format groupid $quick. ;
run;
data temp_main;
set data.main_201203;
where groupid ne '' and groupid ne '0';
where groupid ne '' and groupid ne '0';
keep hhid groupid hh;
run;
data temp_contr;
merge data.contrib_201203 (in=a) temp_main (in=b);
by hhid;
if b;
...
CLASS groupid / PRELOADFMT;
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (dda_con='Checking'*pctsum<hh>='Avg. Con.' mms_con='Mon Mkt'*pctsum<hh>='Avg. Con.' sav_con='Savings'*pctsum<hh>='Avg. Con.' tda_con='Time Dep'*pctsum<hh>='Avg. Con.' ira_con='IRA'*pctsum<hh>='Avg.
...
format groupid $quick. ;
run;
proc tabulate data=data.main_201203 order=data out=segments missing;
where groupid ne '' and groupid ne '0';
where groupid ne '' and groupid ne '0';
var hh;
CLASS groupid segment band ixi_tot cbr distance tenure_yr tran_code/ PRELOADFMT;
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (segment)*N='HHs'*f=comma12. segment*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (tran_code='Tran. Segment')*N='HHs'*f=comma12. tran_code='Tran. Segment'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (ixi_tot='Estimated Wealth')*N='HHs'*f=comma12. ixi_tot='Estimated Wealth'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (cbr)*N='HHs'*f=comma12. cbr*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (distance='Dist to Branch')*N='HHs'*f=comma12. distance='Dist to Branch'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (tenure_yr="tenure (Yrs)")*N='HHs'*f=comma12. tenure_yr="tenure (Yrs)"*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (groupid All) All, sum='All'*hh='HHs'*f=comma12. (band='Profit Band')*N='HHs'*f=comma12. band='Profit Band'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
format groupid $quick. ;
...
set data.main_201203;
where groupid ne '' and groupid ne '0';
keep groupid dda: mms: sav: tda: ira: mtg: iln: ind: heq: card: ccs: sec: hhid ;
...
balance = heq_amt;
output;
keep order balance groupid hhid;
run;
proc format;
...
set data.main_201203;
where groupid ne '' and groupid ne '0';
keep groupid dda mms sav tda ira mtg iln ind heq card sec hhid ;
run;
data cbox;
merge cbox(in=a) data.contrib_201203(in=b keep=hhid dda_con mms_con sav_con tda_con ira_con mtg_con iln_con ind_con heq_con card_con sec_con);
by hhid;
if a;
run;
data cbox;
...
output;
keep order contribution groupid hhid;
run;
proc format;
value order 1 = 'Checking' 2 = 'Money Mkt' 3 = 'Savings' 4 = 'Time Dep' 5 = 'IRAs' 6 = 'Credit Card' 7 = 'Dir. Loan' 8 = 'Ind. Loan' 9 = 'Securities' 10= 'Mortgage' 11='Home Equity';
run;
...
ods pdf file="C:\Documents and Settings\ewnym5s\My Documents\Analysis\ACH\Lost_HEQ_profile_1.pdf" style=mtbnew nogfootnote dpi=300;
...
layout overlay / yaxisopts=(linearopts=( viewmax =50000 viewmin=12500 tickvalueformat=dollar12.) label='Balance' labelattrs=(weight=bold)) xaxisopts=( label="Product" labelattrs=(weight=bold) discreteopts=(TICKVALUEFITPOLICY=STAGGER) ) cycleattrs=true ;
boxplot x=order y=amt1 / discreteoffset=-0.2 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="1" legendlabel="EXISTING LOAN ADJUSTMENT";
boxplot x=order y=amt2 / discreteoffset=-0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="2" legendlabel="NEW PAYMENT";
boxplot x=order y=amt3 / discreteoffset=0 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="3" legendlabel="NO ACH LOAN PAYMENTS";
boxplot x=order y=amt4 / discreteoffset=0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="4" legendlabel="PAYMENT STOP";
boxplot x=order y=amt5 / discreteoffset=0.2 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="5" legendlabel="STEADY STATE";
...
layout overlay / yaxisopts=(linearopts=( viewmax =225000 viewmin=12500 tickvalueformat=dollar12.) label='Balance' labelattrs=(weight=bold)) xaxisopts=( label="Product" labelattrs=(weight=bold) discreteopts=(TICKVALUEFITPOLICY=STAGGER) ) cycleattrs=true ;
boxplot x=order y=amt1 / discreteoffset=-0.2 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="1" legendlabel="EXISTING LOAN ADJUSTMENT";
boxplot x=order y=amt2 / discreteoffset=-0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="2" legendlabel="NEW PAYMENT";
boxplot x=order y=amt3 / discreteoffset=0 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="3" legendlabel="NO ACH LOAN PAYMENTS";
boxplot x=order y=amt4 / discreteoffset=0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="4" legendlabel="PAYMENT STOP";
...
vbar groupid / missing group=proxy groupdisplay=cluster response=col1 nostatlabel grouporder=data datalabel DATALABELATTRS=(Size=6);
...
class groupid;
var hh;
table groupid='Analysis Group' ALL='Total',N='HHs'*f=comma12. pctN='Percent'*f=pctfmt./ nocellmerge ;
format groupid $quick.;
run;
ods pdf close;
