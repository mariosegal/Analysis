...
sheetz <- sqlQuery(myconn,"select * from wip.Sheetz")
sheetz$atm_group <- factor(sheetz$atm_group,labels=c("Sheetz Only","Sheetz and Any Other","Branch Only","Other Partner and Branch","Other Partner Only","No ATM"))
plotdata <- sheetz[(sheetz$atm_group == levels(sheetz$atm_group)[1]| sheetz$atm_group==levels(sheetz$atm_group)[2]) & sheetz$sheetz_num > 0,]
...
library(ggplot2)
library(scales)
p<- ggplot(plotdata,aes(non_sheetz_num,sheetz_num))+stat_binhex(bins=100,binwidth=c(1,1))
p <- p+ scale_x_continuous(limits=c(0,40))+ scale_y_continuous(limits=c(0,40))+geom_abline(slope=1,intercept=0,color="red")
p

plotdata$count <- 1
p<- ggplot(a,aes(non_sheetz_num,sheetz_num,z=sum))+stat_summary_hex()
p <- p+ scale_x_continuous(limits=c(0,25))+ scale_y_continuous(limits=c(0,25))+geom_abline(slope=1,intercept=0,color="red")
p


a<- ddply(plotdata, .(non_sheetz_num, sheetz_num), summarize, sum = sum(count))

p<- ggplot(a,aes(x=non_sheetz_num,y=sheetz_num,size=sum))+geom_jitter(alpha=.7,color="blue")+scale_x_continuous(limits=c(0,25))
