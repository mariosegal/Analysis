...
proc freq data=data.main_201111 ;
table vru_num / missing;
run;

proc univariate data=data.main_201111 ;
var vru_num;
run;
...
where web_signon eq 0;
keep hhid web_signon;
run;

data temp2;
merge data.main_201111 (in=a keep=hhid web_signon rename=(web_signon=nov)) temp1 (in=b);
by hhid;
if b;
run;

proc freq data=temp2;
tables web_signon*nov / missing;
run;
