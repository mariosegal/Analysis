...
keep hhid segment cbr market band rm hh dda mms sav tda ira mtg heq iln ind card sln sec ins trs sdb dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt tag_new;
rename card=ccs;
run;
data temp_contrib;
...
table (tag_new ALL) , (hh dda mms sav tda ira mtg heq iln ind ccs sln sec ins trs sdb)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt
...
con_tot_hh = 0;
output;
keep tag_new product prod_hh penetration balance bal_prod_hh bal_tot_hh contribution con_prod_hh con_tot_hh;
...
if cbr eq . then cbr = 99;
if market eq . then market = 99;
keep tag_new cbr market segment band tenure_yr rm svcs cqi: hh hhid;
run;
proc tabulate data=temp_data out=segment_results missing;
class tag_new segment;
var hh;
table (tag_new ALL) * (segment ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new cbr;
var hh;
table (tag_new ALL) * (cbr ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new market;
var hh;
table (tag_new ALL) * (market ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new band;
var hh;
table (tag_new ALL) * (band ALL), hh*sum*f=comma12. / nocellmerge;
format band $2. ;
run;
data band_results;
set band_results;
...
class tag_new tenure_yr;
var hh;
table (tag_new ALL) * (tenure_yr ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new svcs;
var hh;
table (tag_new ALL) * (svcs ALL), hh*sum*f=comma12. / nocellmerge;
format svcs comma2. ;
run;
data svcs_results;
set svcs_results;
...
class tag_new cqi;
var hh;
table (tag_new ALL) * (cqi ALL), hh*sum*f=comma12. / nocellmerge;
format cqi cqifmt. ;
run;
data cqi_results;
set cqi_results;
...
class tag_new cqi_dd;
var hh;
table (tag_new ALL) * (cqi_dd ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new cqi_bp;
var hh;
table (tag_new ALL) * (cqi_bp ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new cqi_web;
var hh;
table (tag_new ALL) * (cqi_web ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new cqi_deb;
var hh;
table (tag_new ALL) * (cqi_deb ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new cqi_odl;
var hh;
table (tag_new ALL) * (cqi_odl ALL), hh*sum*f=comma12. / nocellmerge;
...
class tag_new rm;
var hh;
table (tag_new ALL) * (rm ALL), hh*sum*f=comma12. / nocellmerge;
format rm $rmfmt. ;
run;
data rm_results;
set rm_results;
...
edeliv_num = 0;
estat_num = 0;
keep tag_new HHID HH web bp WAP SMS edeliv estat fico FWorks web_signon BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num;
...
table (tag_new ALL), (sum)*(HH web bp WAP SMS edeliv estat fico FWorks web_num BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num) / nocellmerge;
run;
data web2;
set web1;
if substr(_type_,1,1) eq 0 then do;
...
keep tag_new service enrolled: active: volume_avg spend_avg;
run;
*************************************************;
...
keep tag_new HHID HH web web1 bp1 bp web_signon BP_NUM BP_AMT vpos: mpos: br_tr: vru: chk: dd: atmo: atmt: cqi_dd ;
...
table (tag_new ALL), (sum)*(HH vpos: mpos: chk: atmo: atmt: dd: bp: ) / nocellmerge;
run;
data tran2;
set tran1;
if substr(_type_,1,1) eq 0 then do;
...
keep tag_new Transaction Enrolled: active: volume_avg spend_avg;
run;
******************************************;
...
set data.main_201206;
where tag_new ne '';
keep tag_new HHID HH clv: ixi: ;
run;
proc tabulate data=temp_clv out=clv1;
where clv_total ne .;
class tag_new ;
var HH clv:;
table (tag_new ALL), N (clv_total clv_rem clv_rem_ten)*mean;
run;
data clv2;
set clv1;
if substr(_type_,1,1) eq 0 then do;
...
where clv_total ne .;
class tag_new clv_total;
var HH ;
table (tag_new ALL) ,(clv_total ALL)*(hh)*(sum rowpctsum<hh>) ;
...
format ;
keep tag_new clv_total HHs percent_hhs;
run;
proc tabulate data=temp_clv out=ixi1;
where ixi_tot ne .;
class tag_new ;
var HH ixi: ;
table (tag_new ALL), N (ixi:)*mean;
run;
data ixi2;
set ixi1;
if substr(_type_,1,1) eq 0 then do;
...
where ixi_tot ne .;
class tag_new IXI_tot;
var HH ;
table (tag_new ALL) ,(ixi_tot ALL)*(hh)*(sum rowpctsum<hh>) ;
...
percent_hhs = sum (of hh_pctsum:);
format ;
keep tag_new ixi_tot HHs percent_hhs;
run;
ods html;
