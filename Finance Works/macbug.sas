...
where hh eq 1;
keep hhid segment cbr market band rm hh dda mms sav tda ira mtg heq iln ind card sln sec ins trs sdb dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt fworks_flag1;
rename card=ccs;
run;
data temp_contrib;
set data.contrib_201206;
...
table (fworks_flag1 ALL) , (hh dda mms sav tda ira mtg heq iln ind ccs sln sec ins trs sdb)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt
mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt)*(mean) (DDA_CON MMS_CON SAV_CON TDA_CON IRA_CON SEC_CON TRS_CON mtg_con heq_con ccs_con ILN_CON SLN_CON ind_con)*sum (DDA_CON MMS_CON SAV_CON TDA_CON IRA_CON SEC_CON TRS_CON mtg_con heq_con
ccs_con ILN_CON SLN_CON ind_con)*mean;
run;
data bal2;
set bal1;
...
con_tot_hh = 0;
output;
keep fworks_flag1 product prod_hh penetration balance bal_prod_hh bal_tot_hh contribution con_prod_hh con_tot_hh;
...
where hh eq 1;
cqi = sum(of cqi:);
keep fworks_flag1 cbr market segment band tenure_yr rm svcs cqi: hh hhid;
run;
proc tabulate data=temp_data out=segment_results missing;
class fworks_flag1 segment;
var hh;
table (fworks_flag1 ALL) * (segment ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 cbr;
var hh;
table (fworks_flag1 ALL) * (cbr ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 market;
var hh;
table (fworks_flag1 ALL) * (market ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 band;
var hh;
table (fworks_flag1 ALL) * (band ALL), hh*sum*f=comma12. / nocellmerge;
format band $2. ;
run;
data band_results;
set band_results;
...
class fworks_flag1 tenure_yr;
var hh;
table (fworks_flag1 ALL) * (tenure_yr ALL), hh*sum*f=comma12. / nocellmerge;
...
var hh;
table (fworks_flag1 ALL) * (svcs ALL), hh*sum*f=comma12. / nocellmerge;
format svcs comma2. ;
run;
data svcs_results;
set svcs_results;
...
class fworks_flag1 cqi;
var hh;
table (fworks_flag1 ALL) * (cqi ALL), hh*sum*f=comma12. / nocellmerge;
format cqi cqifmt. ;
run;
data cqi_results;
set cqi_results;
...
class fworks_flag1 cqi_dd;
var hh;
table (fworks_flag1 ALL) * (cqi_dd ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 cqi_bp;
var hh;
table (fworks_flag1 ALL) * (cqi_bp ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 cqi_web;
var hh;
table (fworks_flag1 ALL) * (cqi_web ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 cqi_deb;
var hh;
table (fworks_flag1 ALL) * (cqi_deb ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 cqi_odl;
var hh;
table (fworks_flag1 ALL) * (cqi_odl ALL), hh*sum*f=comma12. / nocellmerge;
...
class fworks_flag1 rm;
var hh;
table (fworks_flag1 ALL) * (rm ALL), hh*sum*f=comma12. / nocellmerge;
format rm $rmfmt. ;
run;
data rm_results;
set rm_results;
...
edeliv_num = 0;
estat_num = 0;
keep fworks_flag1 HHID HH web bp WAP SMS edeliv estat fico FWorks web_signon BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num;
...
table (fworks_flag1 ALL), (sum)*(HH web bp WAP SMS edeliv estat fico FWorks web_num BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num) / nocellmerge;
run;
data web2;
set web1;
if substr(_type_,1,1) eq 0 then do;
...
keep fworks_flag1 service enrolled: active: volume_avg spend_avg;
run;
*************************************************;
...
keep fworks_flag1 HHID HH web web1 bp1 bp web_signon BP_NUM BP_AMT vpos: mpos: br_tr: vru: chk: dd: atmo: atmt: cqi_dd ;
...
table (fworks_flag1 ALL), (sum)*(HH vpos: mpos: chk: atmo: atmt: dd: bp: ) / nocellmerge;
run;
data tran2;
set tran1;
if substr(_type_,1,1) eq 0 then do;
...
keep fworks_flag1 Transaction Enrolled: active: volume_avg spend_avg;
run;
******************************************;
...
set data.main_201206;
where hh eq 1;
keep fworks_flag1 HHID HH clv: ixi: ;
run;
proc tabulate data=temp_clv out=clv1;
where clv_total ne .;
class fworks_flag1 ;
var HH clv:;
table (fworks_flag1 ALL), N (clv_total clv_rem clv_rem_ten)*mean;
run;
data clv2;
set clv1;
if substr(_type_,1,1) eq 0 then do;
...
where clv_total ne .;
class fworks_flag1 clv_total;
var HH ;
table (fworks_flag1 ALL) ,(clv_total ALL)*(hh)*(sum rowpctsum<hh>) ;
...
format ;
keep fworks_flag1 clv_total HHs percent_hhs;
run;
proc tabulate data=temp_clv out=ixi1;
where ixi_tot ne .;
class fworks_flag1 ;
var HH ixi: ;
table (fworks_flag1 ALL), N (ixi:)*mean;
run;
data ixi2;
set ixi1;
if substr(_type_,1,1) eq 0 then do;
...
where ixi_tot ne .;
class fworks_flag1 IXI_tot;
var HH ;
table (fworks_flag1 ALL) ,(ixi_tot ALL)*(hh)*(sum rowpctsum<hh>) ;
...
percent_hhs = sum (of hh_pctsum:);
format ;
keep fworks_flag1 ixi_tot HHs percent_hhs;
run;
ods html;
