...
table  distance, N='HHs'*f=comma12. (dda1='Checking' mms1='Money Market' sav1='Savings' tda1='Time Deposits' ira1='IRAs' 
...
table distance, N='HHs'*f=comma12. (dda_amt='Checking'*rowpctsum<dda1> mms_amt='Money Market'*rowpctsum<mms1> sav_amt='Savings'*rowpctsum<sav1> tda_amt='Time Deposits'*rowpctsum<tda1> 
...
table distance, segment*rowpctN*f=pctfmt. / nocellmerge misstext='0.0%';
keylabel sum='Total' rowpctsum='';
format products prods. ixi_assets wealthband. state $state. segment hudsonseg. distance distfmtnew.;
run;
...
CLASS  &vars &vars2;
class distance / preloadfmt;
table distance, (&vars ),(&vars2)*rowpctn / nocellmerge;
...
data &group._cross1;
set &group._cross;
where distance ne -1 and distance ne 0;
...
class distance / preloadfmt;
var percent;
table distance, y,x*sum*percent*f=percent8.1;
format x y $productx. distance distfmtnew.;
run;

*create at least one Zero entry per group to avoid missing bars;
data extra;
length x $ 20 y $ 20 ;
array flags{&nvars} &vars;
array flags2{&nvars} &vars2;
...
do i = 1 to 6;
	do j=1 to &nvars;
		do k=1 to &nvars;
		   distance=i-.5;
		   if i eq 5 then distance = 5.5;
		   if i eq 6 then distance = 10.5;
		   percent=0;
		   y=names{j};
		   x=names{k};
		   output;
...
keep x y distance percent;
run;


data combined;
set hudson_cross2 (where=(distance ne -1 and distance ne 0)) extra;
keep x y  percent distance;
run;

data combined;
length distance_name $ 22;
set combined;
distance_name = put(distance,distfmtnew.);
run;
...
class y x distance;
var percent;
table distance*y,x*sum*percent*f=percent6.0;
run;
...
proc tabulate data=hudson.hudson_hh (keep =  products dda: mms: tda: sav: mtg: heq: iln: ccs: ira: ccs: mtx1 distance) 
                missing out=prods_hudson order=data;
WHERE PRODUCTS GE 1;
var products;
class distance / preloadfmt;
class dda1 mms1 tda1 sav1 ira1  iln1  heq1  mtg1  ccs1 mtx1;
table (dda1 mms1 tda1 sav1 ira1  iln1  heq1  mtg1 mtx1 ccs1)*distance*(N sum*products);
format distance distfmtnew.;
run;
...
keep distance prods num;
run;

data prods_hudson;
set prods_hudson extra;
run;
...
class num distance_name;
var prods;
table num,distance_name*sum*prods*f=comma8.1;
run;

options mcompilenote=all;
%macro create_panel_charts1 (xsize=,ysize=, file=, group1=,  order1=, prodfile=, gfmt=);
...
%do i = 1 %to 10;
	...
	%do j = 1 %to 10;
		...
		format percent percent8.0 &group1 &gfmt;
		run;
		quit;
	%end;
	 title1 ;
	 ...
	 axis1 label=(angle=90 f="Arial / bo" justify=center color=white height=14pt "&yname")  minor=none major=none color=white value=none order=(0 to 5 by 0.5); 
	  axis2 label=none  minor=none major=none value=none; *order=(&order1);
	   proc gchart data=&prodfile(where=(num=&i)) gout=work.gseg;
		...
		format prods comma5.1 &group1 &gfmt;
		run;
		quit;
%end;
...
%create_panel_charts1 (xsize=300, ysize=200, file=wip.combined, group1=distance ,
                         order1 = "Under 1 mile" "1 to 2 Miles" "2 to 3 Miles" "3 to 5 Miles" 
                                  "5 to 10 Miles" "Over 10 Miles",prodfile=wip.prods_hudson, gfmt=distfmtnew.)
