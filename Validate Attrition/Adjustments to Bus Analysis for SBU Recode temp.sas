...
data merged_bus;
merge dec_accts_bus (in=a) attr.data_201204 (in=b keep=acct bal hhid open_date rename=(bal=new_bal hhid=hhid_new));
by acct;
if a and b;
run;
...
%do i=1 %to 4;
	...
	Title "Sum of Dec Tracked Accouts as of &month";
...
%cycle
 *##################################################################################;
option mprint mlogic mcompilenote=all;
...
data combined;
set merged_bus_apr (keep = hhid new_bal ptype stype group sbu sbu_new acct rename=(new_bal=bal1)) adjusted_data1 (rename=(sbu=sbu_new bal=bal1));
run;
