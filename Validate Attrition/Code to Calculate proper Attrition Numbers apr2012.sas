...
filename mydata 'C:\Documents and Settings\ewnym5s\My Documents\Validate Attrition\Apr12.txt';
...
infile mydata DLM='09'x firstobs=2  lrecl=4096   dsd ;
	  INPUT hhID $ acct $  ptype $  stype $  sbu $ status $ bal;
run;

filename mydata 'C:\Documents and Settings\ewnym5s\My Documents\Validate Attrition\APR12XTR.txt';
data extra;
length  acct $ 28 source $ 10;
infile mydata DLM='09'x firstobs=2  lrecl=4096   dsd ;
	  INPUT acct $  open_date :mmddyy10. source $;
run;
...
proc sort data=attr.data_201204;
by acct;
run;

data attr.data_201204;
merge attr.data_201204 (in=a) extra (in=b);
by acct;
if a;
run;
...
data temp1;
merge attr.data_201112_new (in=a) attr.clean_grps (in=b keep = hhid group);
by hhid;
if a;
run;
...
bal1 = 0;
if PTYPE in('DDA','SAV','MMS','TDA','IRA')  and SUBSTRN(STYPE,1,1)= 'C' then bal1 = bal;
run;
...
bal1_new = 0;
if PTYPE in('DDA','SAV','MMS','TDA','IRA')  and SUBSTRN(STYPE,1,1)= 'C' and (source = 'WLMT' or (source ne 'WLMT' and date_open ge '01sep2011'd)) then bal1_new = new_bal;
/*bal1_new has only balances for Deposits, otherwise zeros, easier to sum that way on tabulate */
run;


title1 'Mar 2012 Corrected';
...
merge new_hhs(rename=(hhid_new=key ) in=a) attr.data_201204 (in=b rename=(hhid=key));
by key;
if a and b;
run;
...
bal1_new = 0;
if PTYPE in('DDA','SAV','MMS','TDA','IRA')  and SUBSTRN(STYPE,1,1)= 'C' and (source = 'WLMT' or (source ne 'WLMT' and open_date ge '23aug2011'd)) then bal1_new = bal;
run;

title1 'APr 2012 Corrected Bals';
...
*##################################################################################;
* why is it not working in april *
1 .I am going to id the HHs that split and look at them in 201204 and 201201, see why there is so much new money;


data split_hhs;
set hh_lookup;
where hhid ne hhid_new;
run;

proc sort data=split_hhs out=orig (keep=hhid) nodupkey;
by hhid;
run;
...
data a1;
merge attr.data_201204 (in=b rename=(hhid=key) where=(ptype in('DDA','SAV','MMS','TDA','IRA')and substr(stype,1,1)='C')) c (in=a ) ;
if b and a;
run;
...
data x;
set a1;
where source ne 'WLMT';
format bal dollar24. open_date date.;
run;
